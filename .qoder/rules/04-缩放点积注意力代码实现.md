...
# Q, K, V 形状均为 [batch_size, n_heads, seq_len, head_dim]
output = F.scaled_dot_product_attention(                  # 调用原生 SDPA 函数，自动选择最优实现
    query=Q,                                              # 查询张量 Q：[batch, heads, seq_len, d_k]
    key=K,                                                # 键张量 K：[batch, heads, seq_len, d_k]
    value=V,                                              # 值张量 V：[batch, heads, seq_len, d_v]
    attn_mask=None,                                       # 注意力掩码（None 表示不使用掩码）
    dropout_p=0.0,                                        # Dropout 概率（0.0 表示不丢弃）
    is_causal=False                                       # 因果掩码（False 表示非自回归场景）
)
```
