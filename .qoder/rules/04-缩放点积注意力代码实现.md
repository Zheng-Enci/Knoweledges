...
class ScaledDotProductAttention(nn.Module):
    ...
    # 前向传播计算缩放点积注意力
    # 参数: Q 查询矩阵 [batch, heads, seq_len_q, d_k], K 键矩阵 [batch, heads, seq_len_k, d_k], V 值矩阵 [batch, heads, seq_len_v, d_v], mask 可选掩码
    # 返回: output 注意力输出 [batch, heads, seq_len_q, d_v], attention_weights 注意力权重 [batch, heads, seq_len_q, seq_len_k]
    # 示例: output, weights = attention(Q, K, V, mask=None)
    def forward(self, Q, K, V, mask=None):
        # 1. 获取维度 d_k
        d_k = Q.size(-1)
...
# 测试注意力模块
# 参数: 无
# 返回: output 注意力输出 [batch_size, n_heads, seq_len, d_v], weights 注意力权重 [batch_size, n_heads, seq_len, seq_len]
