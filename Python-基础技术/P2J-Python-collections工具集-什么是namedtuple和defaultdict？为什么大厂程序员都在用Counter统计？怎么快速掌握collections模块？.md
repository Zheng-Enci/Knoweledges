# P2J-Python-collections工具集-什么是namedtuple和defaultdict？为什么一线开发者都在用Counter统计？怎么快速掌握collections模块？

## 📝 摘要

collections（集合）模块是什么？namedtuple（命名元组）、defaultdict（默认字典）、Counter（计数器）为什么大厂都在用？本文档用生活化比喻解析这三个强大工具，帮你掌握高效的数据处理技巧，提升代码可读性和性能。
