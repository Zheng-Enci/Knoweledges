...
| 概念 | 说明 |
|------|------|
| 可迭代对象 | 实现了 `__iter__`，可以用 `for` 遍历 |
| 迭代器 | 实现了 `__iter__` + `__next__`，可以逐个取值 |
| 生成器 | 用 `yield` 创建，惰性生成，用到才取值 |
...
| 方式三 | `__getitem__` | 旧式兼容，了解即可 |
| 方式四 | `yield` 生成器函数 | **最推荐**，简洁高效 |
