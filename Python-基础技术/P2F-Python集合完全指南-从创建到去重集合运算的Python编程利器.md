...
集合（Set）是 Python 中一种非常实用的数据结构，核心特点是**无序且唯一**。它最大的用途就是去重，另外还有高效的成员检测和丰富的集合运算（并集、交集、差集等）。本文我们将从零开始，系统学习集合的创建、基本操作、运算方法、去重应用，以及不可变的 frozenset。通过大量代码示例和图表，帮助我们快速掌握集合的各种用法 💪
...
在开始学习集合之前，我们需要先了解一些基础知识，这样学起来会更顺畅 📖

**1. 可迭代对象（Iterable）**

集合可以从可迭代对象创建，比如列表、元组、字符串等。所谓可迭代，就是可以一个一个地取出元素的对象。
...
- **可变对象**：创建后可以修改，如列表、字典、集合
- **不可变对象**：创建后不能修改，如数字、字符串、元组

集合的元素必须是不可变的，这就是为什么列表不能作为集合元素的原因。

**3. 哈希（Hash）**

哈希是一种快速查找技术。Python 通过哈希值来快速定位元素。不可变对象有固定的哈希值，可变对象的哈希值可能会变化。
...
了解了这些前置知识，我们就可以开始学习集合了！在后面学习 frozenset 时，这些概念会再次出现帮助我们理解。
