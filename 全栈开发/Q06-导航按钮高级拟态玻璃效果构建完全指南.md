...
| **Liquid Glass** | Apple 在 WWDC 2025 推出的动态玻璃设计语言，具有折射和高光特性 |
...
### 3.3 Liquid Glass 的高光与折射模拟 / Highlights & Refraction Simulation
...
.nav-btn-liquid::after {
  ...
  background: linear-gradient(
    135deg,
    rgba(120, 180, 255, 0.15) 0%,                       /* 蓝色折射 */
    rgba(180, 120, 255, 0.08) 30%,                      /* 紫色过渡 */
    rgba(255, 120, 200, 0.05) 60%,                      /* 粉色过渡 */
    transparent 80%
  );
  z-index: -1;                                           /* 放在按钮后方 */
  pointer-events: none;
  opacity: 0;                                            /* 默认隐藏 */
  transition: opacity 0.5s ease;                         /* 平滑出现 */
}
...
@media (prefers-color-scheme: light) {
  .nav-bar-liquid {
    /* 亮色多层渐变背景 */
    background:
      radial-gradient(ellipse at 20% 30%, rgba(180, 210, 255, 0.15) 0%, transparent 50%),
      radial-gradient(ellipse at 80% 70%, rgba(220, 180, 255, 0.1) 0%, transparent 50%),
  ...
  .nav-btn-liquid {
    background: rgba(255, 255, 255, 0.5);               /* 亮色下更不透明 */
...
> - **双层伪元素分工明确** — `::before` 负责表面高光，`::after` 负责边缘折射色散 / Two pseudo-elements with distinct roles: `::before` for surface highlights, `::after` for edge refraction
...
玻璃效果 = 背景色彩层(radial-gradient) + 玻璃主体层(backdrop-filter + rgba)
           + 表面高光层(::before 线性渐变) + 边缘光效层(::after / box-shadow)
```
