...
|-------------|------|-------------------|
| **Row Lock** | 行级锁 | 数据库对单行数据施加的锁，确保并发更新的一致性 |
| **Serialization** | 串行化 | 多个并发请求被迫按顺序执行的现象 |
| **Atomic Operation** | 原子操作 | 不可分割的操作，要么完全执行，要么完全不执行 |
| **INCRBY** | 增量自增 | Redis 命令，将键的值增加指定整数 |
| **Write-Behind** | 异步回写 | 先写缓存，再异步批量持久化到数据库的策略 |
| **Eventual Consistency** | 最终一致性 | 系统保证在一段时间后数据达到一致状态，不强求即时一致 |
| **Sharding** | 分片/分表 | 将数据分散到多个数据库实例或表中，打散写热点 |
| **Idempotency** | 幂等性 | 同一操作执行多次与执行一次的结果相同 |
...
**文档来源**：本文档基于抖音视频 [扛住千万人点赞的4层架构设计](https://v.douyin.com/nlWFNZca5ek/) 进行二次解读与技术深化，所有架构设计与代码示例均为原创实现。
