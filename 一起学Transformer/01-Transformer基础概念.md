...
> 💡 **温馨提示**：初学者看到这个图可能会觉得信息量有点大，看不懂很正常！图中涉及的概念（如残差连接、层归一化）将在后续章节详细介绍。此处只需记住：编码器"理解输入"，解码器"生成输出"即可。
...
flowchart LR
    ...
    subgraph 编码器
    E1[嵌入层] --> E2[位置编码] --> E3[多头自注意力 + 残差 + 层归一化] --> E4[前馈网络 + 残差 + 层归一化]
    end
    
    subgraph 解码器
    D1[输出嵌入] --> D2[位置编码] --> D3[掩码注意力 + 残差 + 层归一化] --> D4[交叉注意力 + 残差 + 层归一化] --> D5[前馈网络 + 残差 + 层归一化]
    end
...
|---------|-----|----------|-------------|
| **长距离依赖** | 严重衰减（<50步） | 较好（~200步） | 任意距离，无衰减 |
| **训练速度** | 慢（串行计算） | 较慢 | 快（并行计算） |
| **并行能力** | 差 | 有限 | 强（GPU友好） |
| **计算复杂度** | O(n) | O(n) | O(n²) |
| **位置信息** | 天然隐含 | 天然隐含 | 需额外注入 |
| **内存效率** | 低 | 中 | 较低（注意力矩阵） |

> 💡 **补充说明**：虽然Transformer计算复杂度是O(n²)，但由于并行计算的优势，实际训练速度仍比RNN快10倍以上。对于超长序列，可采用稀疏注意力等优化技术降低复杂度。
