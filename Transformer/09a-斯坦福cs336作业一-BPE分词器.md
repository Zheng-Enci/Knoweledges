...
> 💡 **什么是单字节？**  
> 单字节是指 8 位二进制数（0-255），可以表示 ASCII 字符集中的所有字符。例如：  
> - `0-127`：标准 ASCII（英文字母、数字、标点）  
> - `128-255`：扩展 ASCII（特殊符号、重音字符）  
> 任何 UTF-8 文本都可以拆解为单字节序列，例如 `"abc"` → `[97, 98, 99]`（ASCII 码）

**单字节参考资料**：
- [Byte-level BPE(BBPE)原理及其代码实现 -- 知乎](https://zhuanlan.zhihu.com/p/652520262) ⭐值得阅读
- [一文读懂字符编码 -- 知乎](https://zhuanlan.zhihu.com/p/572877610)
- [常用的字符编码：ASCII、Unicode、UTF-8 -- CSDN](https://blog.csdn.net/yishizuofei/article/details/79622332)
