...
位置编码向量是一个 d_model 维的向量。每个 token 位置都对应这样一个向量。

- **i 的含义**：i 是"频率组"的索引，而不是单个维度的索引
- **每组两个维度**：每个 i 值生成 **两个维度**——偶数维度 2i 用 sin，奇数维度 2i+1 用 cos。这两个维度频率相同，但相位相差 π/2（90°）
- **i 的取值范围**：由于每个 i 产生一对维度，所以 i 的取值是 0 到 d_model/2 - 1。例如当 d_model = 512 时，i 取 0 到 255

举例来说（d_model = 4）：
- i = 0 → 维度 0 用 sin，维度 1 用 cos（高频，波长约 2π）
- i = 1 → 维度 2 用 sin，维度 3 用 cos（中频，波长约 20π）
- i = 2 → 维度 4 用 sin，维度 5 用 cos（低频，波长约 200π）
