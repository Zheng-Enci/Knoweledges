...
但这只是告诉**在哪两个维度填数字**，更重要的是：**不同 i 填入的数字频率不同**！

频率由公式 `10000^(2i/d_model)` 决定：

| i 的值 | 计算公式 | 频率（角度/位置） | 特点 |
|--------|----------|------------------|------|
| i = 0 | 10000^(0/6) = 1 | 1.0 | **高频**，变化极快 |
| i = 1 | 10000^(2/6) ≈ 10 | 0.1 | **中频**，变化适中 |
| i = 2 | 10000^(4/6) ≈ 100 | 0.01 | **低频**，变化很慢 |
...
问题：**位置 1 和位置 101 的值都是 0.84 左右，模型分不清！**

用多种频率后（i = 0, 1, 2 组合）：
```
位置 0:  (0.00, 0.00, 0.00)
位置 1:  (0.84, 0.10, 0.01)
位置 100: (-0.51, 0.84, 0.84)
```
