...
**为什么要用多个不同的频率？（i 的核心作用）** 💡

直接看数字对比：

假设只用一种频率（假设 i = 0），连续几个位置的位置编码值：
```
位置 0: sin(0/1)   = 0.00
位置 1: sin(1/1)   = 0.84
位置 2: sin(2/1)   = 0.91
...
位置 4: sin(4/1)   = -0.76
...
位置 100: sin(100/1) = -0.51
```

问题来了：**位置 1 和位置 101 的值很接近**（都是 0.84 左右），模型分不清它们！

这就是只用一种频率的问题：**不同位置可能得到相似的值**。

**解决方法：用多种频率**

| i 的值 | 位置 0 | 位置 1 | 位置 100 |
|--------|--------|--------|----------|
| i=0 (高频) | 0.00 | 0.84 | -0.51 |
| i=1 (中频) | 0.00 | 0.10 | 0.84 |
| i=2 (低频) | 0.00 | 0.01 | 0.84 |

现在每个位置都有**独特的组合**：位置 0 是 (0.00, 0.00, 0.00)，位置 1 是 (0.84, 0.10, 0.01)，位置 100 是 (-0.51, 0.84, 0.84)。
