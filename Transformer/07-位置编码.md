...
如果序列**很短**（比如只有前 10 个位置），确实每个位置的值都不同。但如果序列**很长**（比如位置 100、107、114...），sin 的周期是 2π ≈ 6.28，超过一个周期后值会**重复**，位置 1 和位置 7（≈ 1 + 2π）的值就很接近！模型分不清远处的位置！
...
![多频率波形分开展示](https://raw.gitcode.com/ZhengEnCi/knoweledges/raw/master/Transformer/07_positional_encoding_wave_superposition.png)

![所有波形叠加展示](https://raw.gitcode.com/ZhengEnCi/knoweledges/raw/master/Transformer/07_positional_encoding_wave_overlay.png)
...
就像不同乐器同时演奏——小提琴（高频）拉得快，大提琴（低频）拉得慢，你听到的是它们的**合声**。每个位置接收到的"合声"独一无二，模型就能识别出是哪个位置。

（多频率设计原理）参考资料：
- [一文搞懂Transformer的位置编码 -- CSDN](https://blog.csdn.net/xian0710830114/article/details/133377460)
...
（为什么 sin/cos 交替）参考资料：
- [Transformer002 位置编码 -- CSDN](https://blog.csdn.net/huizhouboy/article/details/157098955)
...
回顾一下公式中的 i 和 2i：
- **i** 是"维度组"的编号，取值范围是 `0, 1, 2, ..., d_model/2 - 1`
- **2i** 是每组对应的**偶数维度索引**
...
- 维度 0（sin）和维度 1（cos）都除以 `10000^(0/6)`
- 维度 2（sin）和维度 3（cos）都除以 `10000^(2/6)`
- 维度 4（sin）和维度 5（cos）都除以 `10000^(4/6)`
...
- **pow(base, exponent)**：幂运算，计算 `base^exponent`（base 的 exponent 次方）
  - 例如：`pow(10000, 2)` = 10000² = 100,000,000
  
- **exp(x)**：指数函数，计算 `e^x`（e ≈ 2.71828 是自然对数的底数）
  - 例如：`exp(2)` = e² ≈ 7.389
...
**核心原理**：
- 两种方法的**计算结果完全相同**
- 但 `exp-log` 的**计算过程更安全**，不会产生超大的中间值，避免数值溢出
...
![正弦位置编码热力图](https://raw.gitcode.com/ZhengEnCi/knoweledges/raw/master/Transformer/07_chapter5_pe_heatmap.png)
...
> 💡 为什么右侧只有红白没有蓝色？当 `d_model=128` 时，高维度对应的频率极低（分母 `10000^(2i/128)` 超级大），在 `seq_len=100` 的范围内角度变化太小，cos 值一直保持在正值区域。如果要看到完整的红白蓝交替，需要将 `seq_len` 增大到 1000+。
...
![位置编码波形图](https://raw.gitcode.com/ZhengEnCi/knoweledges/raw/master/Transformer/07_chapter5_pe_waveforms.png)
