...
想象一个交响乐团的乐谱，每个乐器（频率）都在演奏，但音高不同：

- **位置 pos** = 乐谱上的时间点（第 1 秒、第 2 秒...）
- **维度索引 i** = 乐团中的**第几件乐器**
- **d_model** = 乐团总共有多少件乐器

位置编码向量就是一个"时刻的交响乐"——每个维度（乐器）发出不同频率的声音，合在一起形成该时刻的独特"声音"。

更具体地说：
- i = 0：第 1 件乐器（高频，变化极快）
- i = 1：第 2 件乐器（稍低频，变化稍慢）
- i = 2：第 3 件乐器（更低频...）
- ...
- i = 255：第 256 件乐器（极低频，几乎不变）

**每个 i 实际产生两个维度**：偶数维度用 sin（正弦波），奇数维度用 cos（余弦波）。这两个维度频率相同，但组合起来能表示更丰富的相位信息——类似于乐队的和声。
