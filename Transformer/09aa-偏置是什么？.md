...
- **第5章 → 第6章**：了解现代大语言模型如何处理偏置（引向后续的 09ab-无偏置线性层）
...
带偏置输出 y = xW^T + b:
tensor([[ 0.1571,  0.2678, -0.0802],
        [-0.5742, -0.2904, -0.3708],
        [ 0.4205,  0.2505,  0.2019]])

无偏置输出 y = xW^T:
tensor([[ 0.0923,  0.1150, -0.2067],
        [-0.6390, -0.4433, -0.4973],
        [ 0.3557,  0.0977,  0.0754]])

差异（偏置的效果）:
tensor([[0.0648, 0.1528, 0.1265],
        [0.0648, 0.1528, 0.1265],
        [0.0648, 0.1528, 0.1265]])
...
------------------------------------------------------------
带偏置     2.0186          2.9947          0.009572     
无偏置     1.9967          0.0000          9.288312     
------------------------------------------------------------
...
- 无偏置模型只能学习到 $w \approx 2.0$，但 $b$ 固定为 0，最终损失高达 9.28（无法拟合截距 3）
...
> 💡 **引申到现代 LLM**：Transformer 中每一层前面都有 LayerNorm / RMSNorm，它会将输入**归一化为零均值、单位方差**。当输入已经是零中心分布时，偏置的补偿功能就被归一化替代了——这就是为什么现代大模型可以在注意力投影层安全地设置 `bias=False`（详见[06. 现代大语言模型中的偏置](#6-现代大语言模型中的偏置)）。
...
> 本章简要介绍偏置在现代 LLM 中的特殊地位

### 6.1 一条引人深思的问题

学习偏置的概念后，你可能会问：既然偏置这么重要，为什么现代大语言模型（如 LLaMA、PaLM、Falcon 等）的线性层都设置为 `bias=False`？

这引出了[09ab-无偏置线性层](09ab-无偏置线性层.md)中详细讨论的问题。简单来说，原因包括：

| 原因 | 说明 |
|------|------|
| **Pre-Norm + 残差连接** | LayerNorm / RMSNorm 的归一化会吸收偏置的偏移效果 |
| **Key 偏置数学冗余** | Softmax 中的 Key 偏置会在 $QK^T$ 点积中被抵消 |
| **训练稳定性提升** | PaLM 论文指出移除 bias 可以 "increase training stability for large models" |
| **减少参数量** | 每层减少 $d_{\text{out}}$ 个参数，大模型堆叠 100+ 层时很可观 |

### 6.2 什么场景需要保留偏置？

并非所有场景都适合移除偏置，以下情况**应该保留偏置**：

- **FFN 中间层**：经过激活函数（ReLU、GELU）前需要偏置
- **最终分类层**：输出 logits 时需要偏置来调整每个类别的偏置
- **CNN 卷积层**：卷积操作通常需要偏置
- **小型模型**：当模型容量较小时，偏置参数是宝贵的表达能力

> 📌 偏置的取舍是工程设计与理论分析的结合——**理解偏置的本质后，才能做出正确的选择**。
...
- [当模型足够大时，Bias项不会有什么特别的作用 -- CSDN](https://blog.csdn.net/ningyanggege/article/details/136698236) ⭐值得阅读
- [Bias项的神奇作用：RoPE + Bias = 更好的长度外推性 -- 科学空间](https://spaces.ac.cn/archives/9577)
...
- [nano-GPT 模型架构介绍 -- GitHub](https://github.com/datawhalechina/code-your-own-llm/blob/main/docs/Chapter05/Architecture.md)
...
| **现代 LLM 做法** | 注意力层用 `bias=False`，FFN 和分类层用 `bias=True` |
