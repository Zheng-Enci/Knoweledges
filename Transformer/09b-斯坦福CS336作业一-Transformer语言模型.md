...
本文档详细解析斯坦福CS336课程作业一的Transformer语言模型架构（Decoder-Only Transformer），涵盖多头注意力、RoPE、RMSNorm、SwiGLU等核心组件的从零实现 🛠️
<!-- 全文摘要结束 -->
...
> 💡 **前置阅读**：BPE分词器的实现请参考 [09a-斯坦福CS336作业一-BPE分词器](https://juejin.cn/post/7643037374348296233)（[CSDN](https://blog.csdn.net/2301_79239314/article/details/161367999)）；课程背景请参考 [09-斯坦福CS336作业](https://juejin.cn/post/7641109461659500582)（[CSDN](https://blog.csdn.net/2301_79239314/article/details/161204079)）。

## 2. Transformer语言模型架构 🏗️

> 本章详细拆解 Transformer 语言模型的核心组件

### 3.1 整体架构

作业一要求实现一个**仅含解码器的 Transformer**（Decoder-Only Transformer），类似 GPT 的结构。
...
class Linear(nn.Module):
    # 初始化权重参数
    # 参数: in_features 输入维度，out_features 输出维度，device 设备，dtype 数据类型
    # 返回: 无
    # 示例: self.weight.shape = (3072, 768)
    ...
    # 前向传播计算线性变换
    # 参数: x 输入张量 [..., in_features]，示例：[batch=2, seq_len=10, in_features=768]
    # 返回: y 输出张量 [..., out_features]，示例：[2, 10, 3072]
    # 示例: y = linear(x)
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # 前向计算：y = x @ W^T（输入形状 ..., in_features → 输出形状 ..., out_features）
        # 数据流动：x[2,10,768] × W^T[768,3072] → y[2,10,3072]
...
本节我们完成了斯坦福 CS336 作业一的核心内容，回顾关键要点：
